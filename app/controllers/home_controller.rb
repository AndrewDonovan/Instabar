class HomeController < ApplicationController
  def index
  	    ...
  	    @instagram = Instagram.user_recent_media("12278927", {:count => 5})

  	    if params[:hashword]
		@tags = Instagram.tag_recent_media((params[:hashword]), {:count => 5, :client_id => ENV['INSTAAPI_CLIENT_ID']})
   	    end

  	    if params[:hashword2]
		@tags2 = Instagram.tag_recent_media((params[:hashword2]), {:count => 5, :client_id => ENV['INSTAAPI_CLIENT_ID']})
   	    end
